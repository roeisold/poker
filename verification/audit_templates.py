...
def audit_file(filepath):
    ...
    if 'index.html' in filepath:
        ...
        if 'playerData?.chipCounts?.[chip]' in content and 'escapeHTML(playerData?.chipCounts?.[chip] || \'0\')' not in content:
            print(f"  FAILED: playerData.chipCounts not escaped in {filepath}")
            return False

        if 'chipValues[chip].toFixed(2)' in content and 'escapeHTML(chipValues[chip].toFixed(2))' not in content:
            print(f"  FAILED: chipValues.toFixed not escaped in {filepath}")
            return False
    ...
    if 'chip_setup.html' in filepath:
        ...
        if 'chipValues[chip]' in content and 'escapeHTML(chipValues[chip])' not in content:
            print(f"  FAILED: chipValues not escaped in {filepath}")
            return False
