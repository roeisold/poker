...
## 2026-04-18 - Reliable Focus for Dynamic Elements
**Learning:** When adding elements to the DOM dynamically, calling `.focus()` immediately may sometimes fail if the browser hasn't finished painting the new elements.
**Action:** Use `setTimeout(..., 0)` to ensure focus is applied in the next event loop tick after the DOM update.
